...
def extract_workbook(filename):
    with zipfile.ZipFile(filename) as z:
        ...
        for idx, s in enumerate(sheets, start=1):
            ...
            if rows_data:
                ...
                for r in rows_data[1:]:
                    ...
                    # Fallback columns if layout varies slightly
                    if not num_atestado:
                        for k, v in r.items():
                            if re.search(r'\d{4,8}/\d{4}', v) or re.search(r'BA\d{6,}', v) or re.search(r'ABC-\d+', v):
                                num_atestado = v.strip()
                            if 'drive.google.com' in v or 'http' in v:
                                link = v.strip()

                    if titulo or num_atestado or link or serv_quant:
                        all_records.append({
                            'sheet': s['name'],
                            'profissional': prof,
                            'funcao': funcao,
                            'objeto': titulo or serv_quant,
                            'titulo': titulo,
        ...
        print(f"\nTotal de registros extraídos de todas as abas: {len(all_records)}")
        with open('scratch_ufc_acervos.json', 'w', encoding='utf-8') as out:
            json.dump(all_records, out, indent=2, ensure_ascii=False)

extract_workbook('scratch_planilha_ufc.xlsx')
